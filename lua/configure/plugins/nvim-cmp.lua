...
cmp.setup(
    ---@diagnostic disable-next-line: redundant-parameter
    {
        ...
        sources = cmp.config.sources(
            {
                ...
                {name = "vsnip"},
                {name = "path"},
                {name = "buffer", trigger_characters = {'.'}, Keyword_length=0},
                {name = "cmdline"},
                -- {name = "cmp_tabnine"},
        ...
        mapping = {
            ...
            [mapping.plugin.nvim_cmp.next_item2] = cmp.mapping(cmp.mapping.select_next_item(), {"i", "s", "c"}),
...
vim.api.nvim_create_autocmd(
    {"TextChangedI"},
    {
        ...
        callback = function()
            ...
            if string.sub(cmp_api.get_cursor_before_line(), -1) == '.' then
                cmp_config.global.completion.autocomplete = {"TextChanged"}
            elseif (vim.fn["vsnip#available"](-1) == 1 or vim.fn["vsnip#available"](1) == 1 or vim.b.visual_multi) then
